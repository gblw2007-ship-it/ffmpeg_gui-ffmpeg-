...
方案 B：源码运行 (开发者模式)
Bash
git clone https://github.com/你的用户名/ffmpeg_gui-ffmpeg-.git
cd ffmpeg_gui-ffmpeg-
pip install customtkinter psutil wmi
python ffmpeg_gui.py
使用指南
选择媒体源：点击“选择并分析”，软件将自动读取视频信息。

配置参数：在选项卡中快速设置编码器（NVENC/H.264/AV1）、分辨率、码率及滤镜。

加入队列：点击“➕ 建立配置并加入队列”。

并发启动：在列表中框选一个或多个任务，点击“▶ 启动选中任务”。

实时监控：通过双栏日志实时观察渲染进度与硬件负载。
